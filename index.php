...
function validatePassword($pass) {
    if (strlen($pass) < 8) return "Hasło musi mieć minimum 8 znaków.";
    if (!preg_match('/[A-Z]/', $pass)) return "Hasło musi zawierać dużą literę.";
    if (!preg_match('/[0-9]/', $pass)) return "Hasło musi zawierać cyfrę.";
    if (!preg_match('/[\W_]/', $pass)) return "Hasło musi zawierać znak specjalny.";
    return true;
}
    ...
    try {
        switch ($action) {
            ...
            case 'read_notes':
                ...
                $tagsMap = []; 
                while($row = $tagsQuery->fetch(PDO::FETCH_ASSOC)) { $tagsMap[$row['note_id']][] = $row['name']; }
                ...
                if ($q === '') {
                    ...
                    foreach ($allNotes as $note) {
                        ...
                        // 1. Prepare raw content: Replace block tags with spaces to prevent word merging (e.g. </div><div>), 
                        //    but keep inline structure intact for things like "logrotate.conf"
                        ...
                        // 2. Strip tags and Decode HTML entities to get pure text representation
                        // This handles &nbsp;, &lt;, etc., and removes <b>, <span>
                        $cleanText = strip_tags($rawContent);
                        $cleanText = html_entity_decode($cleanText, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                        
                        // 3. Build Haystack (Title + Content + Tags)
                        ...
                        // 4. Perform case-insensitive substring search
                        $allMatch = true;
                        foreach ($keywords as $word) {
                            // mb_stripos finds needle ANYWHERE in haystack (substring search)
                ...
                foreach ($filteredNotes as &$note) {
                    ...
                    $d = new DateTime($note['updated_at'] ? $note['updated_at'] : $note['created_at']);
                    $note['display_date'] = $d->format('d.m.Y H:i');
                    unset($note['db_id']);
                }
                $response = ['success' => true, 'notes' => array_values($filteredNotes)];
                break;
            case 'read_note':
                $id = $params['id'] ?? '';
                ...
                } else $response = ['success' => false, 'message' => 'Brak'];
                break;
            case 'save_note':
            ...
            case 'get_metadata': 
                ...
                $response=['success'=>true, 'sections'=>$secs->fetchAll(PDO::FETCH_ASSOC), 'total'=>$total->fetchColumn()]; 
                break;

            case 'save_settings': 
                $pdo->prepare("UPDATE users SET theme=?, accent_color=? WHERE id=?")->execute([$params['theme'], $params['accent_color'], $userId]); 
                $_SESSION['theme']=$params['theme']; $_SESSION['accent_color']=$params['accent_color']; 
                $response=['success'=>true]; 
                break;
            case 'add_section': 
                $pdo->prepare("INSERT INTO sections (user_id,name,color) VALUES (?,?,?)")->execute([$userId, $params['name'], $params['color']]); 
                $response=['success'=>true]; 
                break;
            case 'delete_empty_sections': 
                $pdo->prepare("DELETE FROM sections WHERE user_id=? AND id NOT IN (SELECT DISTINCT section_id FROM notes WHERE section_id IS NOT NULL)")->execute([$userId]); 
                $response=['success'=>true]; 
                break;
            ...
            case 'import_xml': 
                ...
                $c = file_get_contents($_FILES['xml_file']['tmp_name']); 
                $xml = simplexml_load_string($c, 'SimpleXMLElement', LIBXML_NOCDATA); 
                if(!$xml) throw new Exception("Zły XML"); 
                $cnt=0; $secMap=[]; 
                if(isset($xml->sections->section)){
                    $is=$pdo->prepare("INSERT INTO sections(user_id,name,color)VALUES(?,?,?)");
                    $cs=$pdo->prepare("SELECT id FROM sections WHERE user_id=? AND name=?");
                    foreach($xml->sections->section as $s){
                        $cs->execute([$userId,(string)$s['name']]); $eid=$cs->fetchColumn();
                        if($eid)$secMap[(string)$s['id']]=$eid;
                        else{$is->execute([$userId,(string)$s['name'],(string)$s['color']]);$secMap[(string)$s['id']]=$pdo->lastInsertId();}
                    }
                } 
                ...
                foreach($xml->note as $n){
                    $sid=$secMap[(string)$n['section']]??null;
                    $createdRaw = (string)$n['created'];
                    $createdDate = DateTime::createFromFormat('Ymd\THis', $createdRaw);
                    $created = $createdDate ? $createdDate->format('Y-m-d H:i:s') : date('Y-m-d H:i:s');
                    $updatedRaw = (string)$n['modified'];
                    $updatedDate = DateTime::createFromFormat('Ymd\THis', $updatedRaw);
                    $updated = $updatedDate ? $updatedDate->format('Y-m-d H:i:s') : null;
                    $in->execute([uniqid().'-'.substr(str_shuffle('0123456789'),0,6),$userId,$sid,(string)$n['title'],nl2br((string)$n),$created,$updated]);
                    $cnt++;
                } 
                $response=['success'=>true,'imported_count'=>$cnt]; 
                break;
            case 'export_xml': 
                ...
                while($n=$notes->fetch(PDO::FETCH_ASSOC)){$node=$dom->createElement('note');$node->setAttribute('title',$n['title']);
                $c = new DateTime($n['created_at']); $node->setAttribute('created', $c->format('Ymd\THis'));
                if($n['updated_at']){ $m = new DateTime($n['updated_at']); $node->setAttribute('modified', $m->format('Ymd\THis')); }
                if($n['section_id']) { $sname=$pdo->query("SELECT name FROM sections WHERE id=".$n['section_id'])->fetchColumn(); if($sname) $node->setAttribute('section', $sname); }
                $node->appendChild($dom->createCDATASection($n['content']));$root->appendChild($node);} header('Content-Type: application/xml'); header('Content-Disposition: attachment; filename="notes.xml"'); echo $dom->saveXML(); exit; break;
        }
...
.editor-content-wrap { flex: 1; overflow-y: auto; display: flex; flex-direction: column; } 
.content { outline: none; padding: 15px; flex: 1; line-height: 1.5; font-size: 14px; min-height: 200px; }
...
.att-item { display: flex; justify-content: space-between; padding: 2px 5px; background: #eee; margin-bottom: 2px; border-radius: 3px; }
...
<header>
    ...
    <div style="display:flex; gap:10px; font-size:12px;">
        ...
        <?php if($_SESSION['role'] === 'admin'): ?>
            <button onclick="openUsersModal()">Użytkownicy</button>
        <?php endif; ?>
        <button onclick="document.getElementById('settingsModal').style.display='flex'">Opcje</button>
...
<div class="wrap">
    <aside class="sidebar" id="sidebar">
        ...
        <div class="search-box">
            ...
            <button onclick="document.getElementById('q').value=''; readNotes();">⟳</button>
        </div>
        <div style="padding:5px; background:#f0f0f0; border-bottom:1px solid #ccc; display:flex; gap:5px;">
            <button onclick="document.getElementById('sectionModal').style.display='flex'" class="btn-small">+ Sekcja</button>
            <button onclick="deleteEmptySections()" class="btn-small">🧹</button>
            <button onclick="toggleTrashMode()" id="trashBtn" class="btn-small">🗑️ Kosz</button>
            <select id="sortSelect" onchange="readNotes()" style="margin-left:auto; font-size:11px;"><option value="DESC">▼ Data</option><option value="ASC">▲ Data</option></select>
        </div>
    ...
    <main class="editor">
        ...
        <div class="meta-bar" style="padding:5px 15px; background:#f9f9f9; display:flex; gap:10px; font-size:11px;">
            <select id="sectionInput"><option value="">-- Bez sekcji --</option></select>
            <input id="tagsInput" placeholder="Tagi..." style="flex:1; border:1px solid #ddd;">
        </div>
        <div class="editor-tools">
            <button class="sidebar-toggle" onclick="toggleSidebar()">☰ Lista</button>
            <button onclick="newNote()" title="Nowa notatka">📄</button>
            
            <button id="saveBtn" onclick="save()" title="Zapisz" style="background:#e8f0fe;">💾</button>
            
            <div style="width:1px; height:20px; background:#ccc; margin:0 5px;"></div>
            
            <button onclick="document.getElementById('attachInput').click()" title="Załącz plik" style="padding:2px 5px;"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg></button>
            ...
            <button onclick="document.getElementById('imgUploadInput').click()">🖼️</button><input type="file" id="imgUploadInput" style="display:none" accept="image/*">
            <button onclick="resizeImage()">📐</button>
            
            <button onclick="cmd('hiliteColor', '#ffff00')" title="Żółty marker" style="background:#ffff00; border:1px solid #ccc;">🖍</button>
            <button onclick="cmd('hiliteColor', 'transparent')" title="Usuń zaznaczenie" style="border:1px solid #ccc;">🚫</button>

            <div style="width:1px; height:20px; background:#ccc; margin:0 5px;"></div>
            <select onchange="cmd('fontName', this.value);this.value='';"><option value="">Font</option><option value="Arial">Arial</option><option value="Courier New">Courier</option><option value="Verdana">Verdana</option></select>
            <select onchange="cmd('fontSize', this.value);this.value='';"><option value="">Rozm</option><option value="2">Mała</option><option value="3">Norm</option><option value="5">Duża</option></select>
            <input type="color" oninput="cmd('foreColor', this.value)" title="Kolor tekstu" style="width:30px; padding:0;">
            
            <div style="flex:1"></div>
            
            <button id="deleteBtn" onclick="deleteNote()" title="Usuń" style="color:red;">🗑️</button>
            <button id="restoreBtn" onclick="restoreNote()" style="display:none; background:#d4edda; color:green;">♻️</button>
            <button id="permDeleteBtn" onclick="permDeleteNote()" style="display:none; background:#f8d7da; color:red;">❌</button>
            <button id="emptyTrashBtn" onclick="emptyTrash()" style="display:none; background:#666; color:#fff; font-size:10px;">Opróżnij</button>
        </div>
...
<div id="passModal" class="modal"><div class="modal-content"><h3>Hasło</h3><input type="password" id="oldPassInput" placeholder="Stare"><br><input type="password" id="newPassInput" placeholder="Nowe"><br><input type="password" id="confirmPassInput" placeholder="Powtórz"><br><br><button onclick="submitChangePass()">Zmień</button> <button onclick="document.getElementById('passModal').style.display='none'">X</button></div></div>

<div id="usersModal" class="modal"><div class="modal-content" style="width:400px;">
    <h3>Użytkownicy</h3><input type="hidden" id="editUserId">
    <div style="background:#f9f9f9; padding:10px; border:1px solid #eee;">
        <b id="userFormTitle">Dodaj:</b><br>
        <input id="newUserName" placeholder="Login" style="width:100px;">
        <input id="newUserPass" type="password" placeholder="Hasło" style="width:100px;">
        <select id="newUserRole"><option value="user">User</option><option value="admin">Admin</option></select>
        <button onclick="submitUserForm()" id="submitUserBtn">OK</button> <button onclick="cancelEdit()" id="cancelEditBtn" style="display:none">Anuluj</button>
    </div>
    <div id="usersList" style="margin-top:10px; max-height:200px; overflow:auto;"></div>
    <br><button onclick="document.getElementById('usersModal').style.display='none'">Zamknij</button>
</div></div>

<script>
let currentNote=null, currentSectionId='', isTrashMode=false, tempNoteId=null, lastClickedImg=null;
...
function toggleTrashMode() {
    ...
    document.getElementById('trashBtn').style.background = isTrashMode ? '#ffcccc' : '';
    document.getElementById('sectionTabs').style.display = isTrashMode ? 'none' : 'flex';
    
    // Toggle action buttons
    document.getElementById('saveBtn').style.display = isTrashMode ? 'none' : 'inline-block';
    document.getElementById('deleteBtn').style.display = isTrashMode ? 'none' : 'inline-block';
    document.getElementById('restoreBtn').style.display = isTrashMode ? 'inline-block' : 'none';
    document.getElementById('permDeleteBtn').style.display = isTrashMode ? 'inline-block' : 'none';
    document.getElementById('emptyTrashBtn').style.display = isTrashMode ? 'inline-block' : 'none';

    document.getElementById('notesList').innerHTML = '<div style="padding:10px;color:#999">Ładowanie...</div>';
    if(!isTrashMode) currentSectionId = '';
    newNote(); 
    readNotes();
}
...
async function loadNote(id) {
    ...
    if(res.success) {
        ...
        document.getElementById('content').innerHTML = currentNote.content;
        document.getElementById('tagsInput').value = (currentNote.tags||[]).join(', ');
        
        let secVal = currentNote.section_id;
        document.getElementById('sectionInput').value = (secVal !== null) ? secVal : "";

        renderAttachments(currentNote.attachments||[]);
        if(window.innerWidth<=768) document.getElementById('sidebar').classList.remove('active');
        
        ['title','content','sectionInput','tagsInput'].forEach(i => {
            const el = document.getElementById(i);
            if(i === 'content') el.contentEditable = !isTrashMode; else el.disabled = isTrashMode;
        });
...
async function save() {
    ...
    const id = currentNote ? currentNote.id : (tempNoteId || uid());
    const res = await api('save_note', {
        id: id, title: document.getElementById('title').value, content: document.getElementById('content').innerHTML,
        section_id: document.getElementById('sectionInput').value, tags: document.getElementById('tagsInput').value
    });
...
function newNote() {
    if(isTrashMode) return;
    currentNote = null; tempNoteId = uid();
...
async function loadMetadata() {
    ...
    if(res.success) {
        const sel = document.getElementById('sectionInput'); 
        const currentVal = sel.value; 
        sel.innerHTML = '<option value="">-- Bez sekcji --</option>';
        ...
        allDiv.onclick = function() { 
            currentSectionId=''; readNotes(); 
            const t = document.getElementById('sectionTabs').children; for(let i of t) i.classList.remove('active'); this.classList.add('active');
        }; tabs.appendChild(allDiv);
        
        res.sections.forEach(s => {
            sel.innerHTML += `<option value="${s.id}" style="color:${s.color}">${s.name}</option>`;
            ...
            div.onclick = function() { 
                currentSectionId=s.id; readNotes(); 
                const t = document.getElementById('sectionTabs').children; for(let i of t) i.classList.remove('active'); this.classList.add('active');
...
// CLICK TO REMOVE MARKER (FIX)
document.getElementById('content').addEventListener('click', e => { 
    if(e.target.tagName==='IMG') { 
        if(lastClickedImg) lastClickedImg.style.outline='none'; 
        lastClickedImg=e.target; lastClickedImg.style.outline='2px solid blue'; 
    } else { 
        if(lastClickedImg) lastClickedImg.style.outline='none'; lastClickedImg=null; 
    }
...
document.getElementById('attachInput').addEventListener('change', async function(){ if(!currentNote) return alert("Zapisz najpierw!"); let fd=new FormData(); for(let f of this.files) fd.append('files[]',f); fd.append('id',currentNote.id); await api('upload_attachment',fd); loadNote(currentNote.id); });
function renderAttachments(list) { 
    const div = document.getElementById('attachmentsList'); div.innerHTML=''; 
    if(list.length===0) div.innerHTML='<span style="color:#aaa">Brak załączników</span>';
    list.forEach(a => div.innerHTML+=`<span class="att-item"><a href="uploads/${currentNote.id}/${a.name}" target="_blank">📄 ${a.name}</a> <span style="color:red;cursor:pointer;margin-left:5px;" onclick="delAtt('${a.name}')">x</span></span> `); 
}
...
function deleteEmptySections() { if(confirm("Usunąć puste sekcje?")) api('delete_empty_sections').then(r=>{alert(r.message||"OK"); loadMetadata();}); }
function openSettingsModal() { document.getElementById('settingsModal').style.display='flex'; }
function openChangePassModal() { document.getElementById('passModal').style.display='flex'; }
function openAddSectionModal() { document.getElementById('sectionModal').style.display='flex'; }
...
function openExportModal() { 
    const sel = document.getElementById('exportSectionInput'); sel.innerHTML='<option value="">Wszystkie</option>';
    document.querySelectorAll('#sectionInput option').forEach(o => { if(o.value) sel.innerHTML+=`<option value="${o.value}">${o.text}</option>`; });
...
function saveAppearance() {
    const th = document.querySelector('input[name="theme"]:checked').value; const col = document.getElementById('accentColorInput').value;
    api('save_settings',{theme:th, accent_color:col}).then(()=>location.reload());
}
function submitChangePass() {
    api('change_password',{old_password:document.getElementById('oldPassInput').value, new_password:document.getElementById('newPassInput').value}).then(r=>{ alert(r.message||(r.success?"OK":"Błąd")); if(r.success) location.reload(); });
}
...
async function submitUserForm() {
    const id = document.getElementById('editUserId').value; const u = document.getElementById('newUserName').value; const p = document.getElementById('newUserPass').value; const r = document.getElementById('newUserRole').value;
    let action = 'add_user'; let data = {username: u, password: p, role: r};
    if (id) { action = 'edit_user'; data.target_id = id; } else { if (!p) { alert("Hasło!"); return; } }
    const res = await api(action, data); if (res.success) { loadUsers(); cancelEdit(); } else alert(res.message);
}
async function adminPass(id) { let p = prompt("Nowe hasło:"); if(p) api('admin_change_pass', {target_id: id, new_password: p}).then(r=>alert(r.message||"OK")); }
async function delUser(id) { if(confirm("Usunąć?")) api('delete_user', {target_id: id}).then(()=>loadUsers()); }

(async function(){ 
    document.execCommand('styleWithCSS', false, true); 
    newNote(); await loadMetadata(); await readNotes(); 
})();

document.getElementById('searchBtn').onclick = readNotes;
document.getElementById('refreshBtn').onclick = () => { document.getElementById('q').value=''; readNotes(); };
document.getElementById('q').addEventListener('keypress', e => { if(e.key==='Enter') readNotes(); });
</script>
<?php endif; ?>

<?php if(!isset($_SESSION['user_id'])): ?>
<script>document.getElementById('loginBtn').onclick = async () => { let r = await fetch('?action=login', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:`action=login&username=${encodeURIComponent(document.getElementById('loginUser').value)}&password=${encodeURIComponent(document.getElementById('loginPass').value)}`}).then(d=>d.json()); if(r.success) location.reload(); else document.getElementById('loginError').innerText = r.message; }</script>
<?php endif; ?>
</body>
</html>
